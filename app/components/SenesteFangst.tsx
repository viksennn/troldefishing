...
const SenesteFangst: React.FC = async () => {
    ...
    if (data.length > 0) {
      ...
      const latestCatches = allCatches.slice(0, 3);
      ...
      <>
        <div className='flex flex-col gap-6 items-center'>
          {latestCatches.map((latestCatch) => (
            <div key={latestCatch.catch._id} className='flex flex-row gap-10 items-center border p-5 rounded lg:full lg:h-[250px] w-full justify-between'>
              <div>
                <div className='flex flex-col gap-3'>
                  <Link href={`/fiskerne/${latestCatch.id}`} className='text-xl font-bold hover:underline'>
                    {latestCatch.fisherman}
                  </Link>
                  <FishTypeCard art={latestCatch.catch.art} className="h-16"/>
              ...
              {latestCatch.catch.imgUrl && (
                <div>
                  <FiskeBillede img={latestCatch.catch.imgUrl} trigger={latestCatch.catch.imgUrl && <img src={latestCatch.catch.imgUrl} alt={latestCatch.catch.art} className='rounded max-w-[200px] w-[125px] lg:w-[150px]' />}/>
                </div>
              )}
              {!latestCatch.catch.imgUrl && (
